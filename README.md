...
Navigate to the Admin app and log in with your username or email. If you've forgotten your password, use the "Forgot password?" link — a reset link is emailed to you and expires in 60 minutes.

![Login](docs/screenshots/login.png)

### Dashboard — campus at a glance

The Dashboard is the default landing page after login. It shows:
- **Building occupancy cards** — click either to instantly filter the table below to that building
- **A status donut chart** — the same room-status breakdown as the summary cards, visualized
- **"Next Up, Campus-Wide"** — the next few sessions starting today, anywhere on campus, independent of the table's own filters
- **A filterable, searchable room table** — filter by building/floor/side/status or search by module/lecturer name

![Dashboard](docs/screenshots/dashboard.png)

### Managing Campus Structure

Rooms are managed here, grouped by building tab. Room codes are auto-generated by the system based on the building/floor/side/type you select — they're never typed manually, which guarantees the naming convention (`M-05A-L01` etc.) is always followed correctly.

![Campus Structure](docs/screenshots/campus-structure.png)

### Managing Sessions

The core scheduling screen. Every session shows a status-appropriate set of actions:
- **Scheduled / Rescheduled** → Edit, Reschedule, Cancel (Rescheduled sessions also get a "View History" button showing the full chain of changes)
- **Cancelled** → Reopen (blocked if the original slot has since been taken by another session)
- **Completed** → no actions (the system automatically marks a session Completed once its end time passes)

Creating or rescheduling a session automatically checks for room *and* lecturer conflicts before saving.

![Sessions](docs/screenshots/sessions.png)
![New Session Modal](docs/screenshots/session-new-modal.png)

### Managing Digital Signage Displays

Register a physical display here, assign it to a building/floor/side, and set how long each slide shows before advancing (default: 8 seconds). The **Device ID** you register here is what you'll use in the Signage app's URL (see "Deploying to a real screen" below).

![Displays](docs/screenshots/displays.png)

### Notifications

Create short announcements that scroll across the bottom of every Digital Signage display campus-wide, independent of any specific session. Only **Active** notifications are shown; deactivating or deleting one removes it from displays within the next ~60-second poll.

![Notifications](docs/screenshots/notifications.png)

### Settings

- **My Profile** — Super Admins can edit their own name/email directly; regular Admins submit a change request instead, which a Super Admin must approve.
- **Admin Accounts** *(Super Admin only)* — create, edit, deactivate/reactivate other admin accounts, and change roles. New accounts receive a real emailed activation link.
- **Pending Requests** *(Super Admin only)* — review and approve/reject profile-change requests submitted by Admins.

![My Profile](docs/screenshots/settings-profile.png)
![Admin Accounts](docs/screenshots/settings-admin-accounts.png)
![Pending Requests](docs/screenshots/settings-pending-requests.png)

### The Digital Signage display

This is the public, unauthenticated app meant to run full-screen on an actual TV or monitor. It automatically cycles through Ongoing → Upcoming → Cancelled (if any) → Rescheduled (if any), refreshing its data roughly every 60 seconds.

![Signage - Ongoing](docs/screenshots/signage-ongoing.png)
![Signage - Notification Ticker](docs/screenshots/signage-notification-ticker.png)

### Deploying to a real screen

The Digital Signage app is built once and deployed as a single instance — each physical screen doesn't need its own separate build. Instead, each screen is pointed at a URL that identifies which display it is: "https://your-deployed-signage-url.com/?device=DSP-0012"

Where `DSP-0012` matches a **Device ID** you registered in the Admin app's Displays page. In practice, this means: open that URL in a browser running in kiosk/full-screen mode on the TV (most smart TVs, or a small device like a Raspberry Pi connected to any screen, can do this), and bookmark or auto-launch it on boot. No rebuild or code change is needed to add more screens — just register a new display in the Admin app and open its own URL on the new screen.
...
| ------------ | ----------------------- | ----------- |
| `disura.s`   | `SuperAdminPass123!` | Super Admin |
| `test.admin` | `AdminPass123!`          | Admin       |
